...
class HandshakeMessage(object):
    ...
    def from_bytes(cls, bytes):
        if bytes[0] == HandshakeMessage.ClientHello:
            self = ClientHelloMessage()
        elif bytes[0] == HandshakeMessage.CertificateStatus:
            self = CertificateStatusMessage()
        elif bytes[0] == HandshakeMessage.ServerHello:
            self = ServerHelloMessage()
        elif bytes[0] == HandshakeMessage.ServerKeyExchange:
            self = ServerKeyExchangeMessage()
        elif bytes[0] == HandshakeMessage.Certificate:
...
class ClientHelloMessage(HandshakeMessage):
    ...
    def create(cls, client_version, random,
        ...
        message = struct.pack(b'!H32sB%ds%ds%ds' % (len(ciphers), len(compression), len(ext)),
                              client_version,
                              random,
                              0, # sessionid length,
                              ciphers,
...
class CertificateStatusMessage(HandshakeMessage):
    ...
    def status_type(self):
        return self.bytes[self.STATUS_OFFSET]
