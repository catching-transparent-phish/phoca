...
def read_tls_record(f):
    ...
    if typ == TLSRecord.Handshake:
        logger.debug('>>> Handshake message: %s', HandshakeMessage.message_types.get(pay[0], 'UNKNOWN!'))
    elif typ == TLSRecord.Alert:
        logger.debug('>>> Alert message: %s', AlertMessage.alert_types.get(pay[1], 'UNKNOWN!'))
